...
<%ArrayList<CustomerEntity> listCust = (ArrayList<CustomerEntity>) request.getAttribute("listCust");%>
<form action="/resultCustomer">
  <h2>Изменить сведения о должности</h2>
  <input name="del" value="false" type="hidden">
  <input name="id" value="<%=listCust.get(0).getId()%>" type="hidden">
  <div class="divNewCustText">
    <p>ФИО:</p>
    <p>Телефон:</p>
  </div>
  <div class="divNewCustData">
    <p><input type="text" size="30px" name="name" value="<%=listCust.get(0).getName()%>"></p>
    <p><input type="text" size="7px" name="phone" value="<%=(listCust.get(0).getPhone()==null)?"":listCust.get(0).getPhone()%>"></p>
  </div>
  <button class="newCustButton" type="submit"><b>Изменить</b></button>
</form>
<%}%>
</body>
</html>
