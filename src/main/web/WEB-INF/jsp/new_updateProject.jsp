...
<form action="/new_updateProject">
  <h2>Новый Проект</h2>
  <input name="new" value="true" type="hidden">
  <input name="res" value="true" type="hidden">
  <div class="divNewProText">
    <p>Название:</p>
    <p>Заказчик:</p>
    <p>Дата начала:</p>
    <p>Дата окончания:</p>
  </div>
  <div class="divNewProData">
  ...
  <button class="newProButton" type="submit"><b>Добавить</b></button>
</form>
<%if (res.equals("true")){%>
<table id="resultNewTable">
  <caption>Новый проект успешно добавлен</caption>
  <tr>
    <th>id</th>
    <th>Название</th>
    <th>Заказчик</th>
...
<form action="/resultProject?auto=false&del=false">
  ...
  <div class="divNewProData">
    <p><input type="text" size="30px" name="name" value="<%=listPro.get(0).getName()%>"></p>
    <p><select name="customer">
      <%ArrayList<CustomerEntity> listCust = (ArrayList<CustomerEntity>) request.getAttribute("listCust");
        for (int i = 0; i < listCust.size(); i++) {
          if (listPro.get(0).getCustomer().getId() == listCust.get(i).getId()){%>
  ...
  <button class="newProButton" type="submit"><b>Изменить</b></button>
</form>
<%}%>
</body>
</html>
